...
FRED_EXPECTATIONS = [
    Expectation(
        "no_nulls", lambda df: df["value"].notna().all(), "Values must not be null"
    ),
    Expectation(
        "cpi_range",
        lambda df: (
            df[df.series_id == "CPIAUCSL"]["value"].between(0, 500).all()
            if "CPIAUCSL" in df.series_id.values
            else True
        ),
        "CPI must be 0–500",
    ),
    Expectation(
        "unrate_range",
        lambda df: (
            df[df.series_id == "UNRATE"]["value"].between(1, 30).all()
            if "UNRATE" in df.series_id.values
            else True
        ),
        "Unemployment must be 1–30%",
    ),
    Expectation(
        "date_not_future",
        lambda df: pd.to_datetime(df["observation_date"]).max() <= pd.Timestamp.today(),
        "No future dates",
    ),
    Expectation(
        "no_duplicates",
        lambda df: not df.duplicated(["series_id", "observation_date"]).any(),
...
MARKET_EXPECTATIONS = [
    Expectation(
        "prices_positive",
        lambda df: (df[["open", "high", "low", "close"]] > 0).all().all(),
    ...
    Expectation(
        "no_nulls",
        lambda df: df[["open", "high", "low", "close"]].notna().all().all(),
        "No null OHLC values",
    ),
]
...
OIL_GOLD_EXPECTATIONS = [
    Expectation(
        "oil_range",
        lambda df: (
            df[df.symbol.isin(["CLF", "BZF"])]["close"].between(10, 300).all()
            if not df[df.symbol.isin(["CLF", "BZF"])].empty
            else True
        ),
        "Oil price $10–$300",
    ),
    Expectation(
        "gold_range",
        lambda df: (
            df[df.symbol == "GCF"]["close"].between(500, 5000).all()
            if not df[df.symbol == "GCF"].empty
            else True
        ),
        "Gold price $500–$5000",
    ),
...
def run_checkpoint(
    df: pd.DataFrame, expectations: list[Expectation], series_name: str
) -> CheckpointResult:
